                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="box-content">
                            ...
                            <a class="btn btn-sm btn-info" href="{{ route('India_Add') }}">Add
                                    India Project
                                </a>
                            </div>
                            <table id="example" class="table table-striped table-bordered display" style="width:100%">
                                ...
                                <tbody>
                                    @foreach($india_state as $banner)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                        <td>{{ $banner->title }}</td>
                                        <td>
                                            <a href="{{ route('India_Action',['id' => $banner->id]) }}"
                                            ...
                                            <form id="update-status-form-{{ $banner->id }}"
                                                action="{{ route('India_Action',['id' => $banner->id]) }}"
                                                method="POST" style="display: none;">
                                                @csrf
                                            </form>
                                        </td>
                                        <td>
                                            <a href="{{ route('India_Edit', ['id' => $banner->id] ) }}"><i
                                                    class="fa fa-edit" style="color:blue"></i></a>
                                            <a onclick="return confirm('Are you sure?')"
                                                href="{{ route('India_Destory',['id' => $banner->id]) }}"><i
